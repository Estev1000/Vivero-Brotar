...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let inventory = JSON.parse(localStorage.getItem('repuestospos_inventory')) || [
        ...
        { id: 8, code: 'SUS-002', name: 'Sustrato Cactus 5L', brand: 'Sustratos Plus', category: 'Sustratos', price: 650, stock: 20 },
    ...
    let config = JSON.parse(localStorage.getItem('repuestospos_config')) || {
        storeName: 'Brotar',
        storeSlogan: 'Tu rincón verde',
        taxId: '',
        address: '',
        taxPercent: 21
    };

    if (config.storeName === 'Hielo del Monte' || config.storeName === 'WaterPoint POS') {
        config.storeName = 'Brotar';
    }
    let cart = [];
    ...
    function getRepairStatusLabel(s) {
        const labels = {
            ...
            waiting_parts: 'EN ESPERA DE STOCK',
    ...
    function renderClientView(data) {
        ...
        const steps = [
            ...
            { k: 'working', l: 'EN PREPARACIÓN' },
            { k: 'waiting_parts', l: 'EN ESPERA DE STOCK' },
            { k: 'ready', l: '¡LISTO PARA ENTREGA!' },
    ...
    function loadSettings() {
        ...
        const storeSloganInput = document.getElementById('set-store-slogan');
        if (storeNameInput) storeNameInput.value = config.storeName || '';
        if (storeSloganInput) storeSloganInput.value = config.storeSlogan || '';
    ...
    document.getElementById('settings-form').onsubmit = (e) => {
        ...
        const storeNameInput = document.getElementById('set-store-name');
        const storeSloganInput = document.getElementById('set-store-slogan');
        if (storeNameInput) config.storeName = storeNameInput.value.trim() || 'Brotar';
        if (storeSloganInput) config.storeSlogan = storeSloganInput.value.trim() || 'Tu rincón verde';
    ...
    function applyBranding() {
        ...
        const bannerTitle = document.querySelector('.hero-banner .banner-text h2');
        const bannerP = document.querySelector('.hero-banner .banner-text p');
        ...
        if (dashboardView.style.display !== 'none') {
            ...
            if (bannerTitle) bannerTitle.textContent = `Panel de ${config.storeName}`;
            if (bannerP) bannerP.textContent = config.storeSlogan;
        }
    }
